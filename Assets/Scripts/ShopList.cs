...
public class ShopList : MonoBehaviour
{
     ...
     * ItemDatabse가 아닌 ShopDatabase에서 아이템 목록들을 불러옵니다.
     */
    ...
    void Start()
    {
        for (int i = 0; i < ShopDatabase.instance.items.Count || i < 4; i++)
        {
            for (int j = 0; j < 1; j++)
            {
                ...
                newSlot.GetComponent<Slot>().number = i;

                ItemImageChange(slotScripts[i]); // 모든 슬롯의 이미지 오브젝트 비활성화 (아이템이 슬롯에 있다면 반복문 아래의 AddItem을 통해 활성화)
            }
        }

        for (int i = 0; i < ShopDatabase.instance.items.Count; i++)
        {
            AddItem(i);
        }
    }
    void AddItem(int number)
    {
        ...
        {
            slotScripts[number].item = ShopDatabase.instance.items[number];
    ...
    public void AddToDatabase()
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            if (slotScripts[i].item.name != "")
            {
                ...
                ShopDatabase.instance.Add(slotItem.name, slotItem.value, slotItem.price, slotItem.description, slotItem.itemType);
            }
        }
    }
}
